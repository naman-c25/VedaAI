  ...
  for (const model of MODELS) {
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        if (problem) throw new Error(`${model} try${attempt}: ${problem}`);
        return parsed;
      } catch (error) {
        lastError = error;
        ...
        if (attempt === 2) {
          console.warn(`[gemini] ${model} failed twice, moving on — ${error.message}`);
          break;
        }
      }
    }
...
export async function extractAnswers(pages) {
  ...
  if (blocks.length === 0) {
    ...
    console.warn(
      `[gemini] ${blocks.length} blocks returned but none carried a usable box_2d. ` +
        `first=${JSON.stringify(blocks[0]).slice(0, 200)}`
    );
  }

  return { blocks: located, student: { name, roll } };
}
...
9. "feedback" is 2-4 sentences written FOR THE TEACHER, about the student in the THIRD PERSON.
   A teacher is reading this screen, not the student, so never address the student as "you".
   Refer to them by the name given in STUDENT below - "Rahul identified...", "Rahul did not
   name..." - and if no name was given, write "The student". When a pronoun is unavoidable use
   "they"/"their": never infer the student's gender from their name.
...
WHOLE-PAPER SUMMARY:
12. "overallFeedback" is 3-5 sentences for the TEACHER, in the same third-person voice and using
    the same name: what the student genuinely understands, where marks were lost, and any pattern
    across questions. Use subject vocabulary.
...
export async function mapAndGrade(questions, blocks, student = {}) {
  const payload = {
    // Named so the grader writes about this student rather than to them.
