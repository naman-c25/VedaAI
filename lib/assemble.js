...
/**
 * Every form a question's number could plausibly be written in on the sheet.
 *
 * Matching on the printed label alone was too brittle for sub-parts: a paper that
 * prints "(a)" under a "7." stem yields label "(a)" -> "a", which never matches a
 * student's "Q7(a)" -> "7a". Building the key from number+subpart as well closes
 * that gap, and it is exactly the case that kept breaking.
 */
export function questionKeys(q) {
  const keys = new Set();
  keys.add(normalizeLabel(q.label));
  if (q.number != null) {
    const n = normalizeLabel(String(q.number));
    keys.add(q.subpart ? n + normalizeLabel(String(q.subpart)) : n);
  }
  keys.delete("");
  return keys;
}

/** A label that is only a sub-part marker: "a", "b", "ii", "iv". */
const BARE_SUBPART = /^(?:[a-h]|i{1,3}|iv|vi{0,3}|ix|x)$/;

/**
 * The keys a written answer label could match. A student who writes "Q10" then
 * "(ii)" underneath means 10(ii); carrying the last parent number forward
 * recovers that, which the model's own matching frequently misses.
 */
export function blockKeys(label, carriedParent) {
  const raw = normalizeLabel(label);
  const keys = new Set();
  if (!raw) return keys;
  keys.add(raw);
  if (carriedParent && BARE_SUBPART.test(raw)) keys.add(carriedParent + raw);
  return keys;
}

/** The parent number a label establishes for anything written after it. */
export function parentOf(label) {
  const raw = normalizeLabel(label);
  const m = /^(\d+)/.exec(raw);
  return m ? m[1] : null;
}
...
export function assemble(questions, blocks, mapping, student = {}, highlightsUnavailable = false) {
  ...
  for (const q of questions) {
    for (const key of questionKeys(q)) {
      if (!questionByNorm.has(key)) questionByNorm.set(key, q);
    }
  }

  // Safety net: an unclaimed block whose written label identifies a question.
  // Runs in sheet order so a bare "(ii)" can inherit the parent number written
  // above it. This is deterministic — sub-parts must never depend on the model
  // having matched them.
  const rescued = new Map(); // questionId -> [blockId]
  let carriedParent = null;
  for (const b of blocks) {
    const parent = parentOf(b.label);
    if (parent) carriedParent = parent;
    if (claimed.has(b.id) || !b.label) continue;

    let q = null;
    for (const key of blockKeys(b.label, carriedParent)) {
      q = questionByNorm.get(key);
      if (q) break;
    }
    if (!q) continue;
  ...
  const assembled = questions.map((q) => {
    ...
    const owned = ids
      ...
      .sort((a, b) => a.page - b.page || (a.rect?.top ?? 0) - (b.rect?.top ?? 0));
...
export function summarise(questions, unmatched, narrative = {}) {
  ...
  return {
    ...
    highlightsUnavailable: Boolean(narrative.highlightsUnavailable),
