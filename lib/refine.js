...
export function refineRect(mask, rect) {
  ...
  const touching = bands.filter(([a, b]) => b >= oTopRel - tol && a <= oBotRel + tol);
  if (touching.length === 0) return rect;

  // An oversized or offset box can graze the answer below it. Bare overlap would
  // then merge the two, producing a confident highlight over the wrong answer —
  // the worst failure mode here. Require a band to be *substantially* inside the
  // box before it counts: the lines of one answer easily clear this, a neighbour
  // the box only clips does not. Falls back to bare overlap if that leaves none.
  const substantial = touching.filter(([a, b]) => {
    const overlap = Math.min(b, oBotRel) - Math.max(a, oTopRel) + 1;
    return overlap >= 0.25 * (b - a + 1);
  });
  const kept = substantial.length > 0 ? substantial : touching;
